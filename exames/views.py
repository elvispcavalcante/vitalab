...
def solicitar_exames(request):
    tipos_exames = TiposExames.objects.filter(disponivel=1)
    ...
    if request.method == 'GET':
        ...
        solicitacao_exames = TiposExames.objects.filter(id__in=exames_id, disponivel=1)
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        if not exame.resultado:
            messages.add_message(request, constants.ERROR, 'Ainda não foi cadastrado o seu exame. Fale com o laboratório')
